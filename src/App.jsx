...
function SocialIcon({ label, href }) {
  return <a aria-label={label} title={label} href={href} target="_blank" rel="noreferrer" className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-sm font-black text-white transition hover:bg-yellow-500 hover:text-slate-950">{label.charAt(0)}</a>;
}
...
function CalendlyEmbed() {
  return (
    <iframe
      ...
      className="h-[760px] w-full rounded-[2rem] border-0 bg-white shadow-2xl"
      loading="lazy"
    />
  );
}
        ...
        <section className="relative overflow-hidden px-6 py-20 md:py-28">
          ...
          <div className="mx-auto grid max-w-7xl items-center gap-12 md:grid-cols-2">
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
              ...
              <h1 className="text-4xl font-extrabold leading-tight tracking-tight md:text-6xl">Helping people and businesses grow through coaching, training, ecommerce, and smart solutions.</h1>
              <p className="mt-6 max-w-xl text-lg leading-8 text-slate-600">J-LEE FANTASY SERVICES LLC provides business coaching, logistics support, ecommerce guidance, AI certification, workplace safety training, credit management, and more.</p>
            ...
            <div className="grid gap-10 lg:grid-cols-2">
              <div>
                ...
                <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                  <LinkButton href={EMAIL_URL} className="bg-yellow-500 text-slate-950 hover:bg-yellow-400">Request Course Info</LinkButton>
            ...
            <div className="grid items-center gap-10 lg:grid-cols-2">
              ...
              <Card className="border-0 bg-gradient-to-br from-blue-900 to-slate-950 text-white shadow-2xl">
                <CardContent className="p-8">
                  ...
                  <LinkButton href="#pricing" className="mt-6 bg-yellow-500 text-slate-950 hover:bg-yellow-400">View Service Packages</LinkButton>
                </CardContent>
              </Card>
            </div>
          </div>
                ...
                <Card key={pkg.name} className={`relative border shadow-lg ${pkg.featured ? "border-yellow-400 shadow-2xl" : "border-slate-100"}`}>
                  ...
                  <CardContent className="p-7">
                    ...
                    <div className="mt-6 space-y-3">
                      {pkg.items.map((item) => <div key={item} className="flex items-start gap-2 text-sm text-slate-700"><CheckCircle className="mt-0.5 text-yellow-600" size={18} /><span>{item}</span></div>)}
                    </div>
                    <LinkButton href="#booking" className={`mt-7 w-full ${pkg.featured ? "bg-yellow-500 text-slate-950 hover:bg-yellow-400" : ""}`}>Choose Package</LinkButton>
                  </CardContent>
                </Card>
              ))}
            </div>
        ...
        <section id="booking" className="relative overflow-hidden px-6 py-20">
          ...
          <div className="mx-auto max-w-7xl">
            <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="mx-auto max-w-3xl text-center text-white">
              <div className="mx-auto mb-5 inline-flex items-center gap-2 rounded-full border border-yellow-400/40 bg-white/10 px-5 py-2 text-sm font-bold text-yellow-300 backdrop-blur"><Sparkles size={16} /> Luxury Booking Experience</div>
              <h2 className="text-4xl font-black tracking-tight md:text-6xl">Schedule Your Consultation</h2>
              <p className="mt-5 text-lg leading-8 text-slate-300">Choose the best time to discuss your business vision, ecommerce goals, AI training needs, logistics support, workplace safety training, or credit management plan.</p>
            </motion.div>

            <div className="mt-10 grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
              <div className="space-y-5">
                <Card className="border border-white/10 bg-white/10 text-white shadow-2xl backdrop-blur">
                  <CardContent className="p-7">
                    <CalendarDays className="mb-4 text-yellow-400" size={40} />
                    <h3 className="text-2xl font-black">What to expect</h3>
                    <div className="mt-5 space-y-4 text-slate-200">
                      <div className="flex gap-3"><CheckCircle className="mt-1 text-yellow-400" size={20} /> A professional consultation based on your goals.</div>
                      <div className="flex gap-3"><CheckCircle className="mt-1 text-yellow-400" size={20} /> Clear recommendations for your next step.</div>
                      <div className="flex gap-3"><CheckCircle className="mt-1 text-yellow-400" size={20} /> Support for ecommerce, AI, logistics, safety, business, or credit needs.</div>
                    </div>
                  </CardContent>
                </Card>
                <Card className="border border-white/10 bg-white/10 text-white shadow-2xl backdrop-blur">
                  <CardContent className="p-7">
                    ...
                    <p className="mt-3 text-slate-300">You can also reach us directly by phone, WhatsApp, or email.</p>
                    <div className="mt-5 flex flex-col gap-3">
                      <LinkButton href={WHATSAPP_URL} external className="bg-yellow-500 text-slate-950 hover:bg-yellow-400">WhatsApp Us</LinkButton>
                      <LinkButton href={EMAIL_URL} variant="outline">Email Us</LinkButton>
                    </div>
                  </CardContent>
                </Card>
              </div>
              <div className="rounded-[2rem] border border-yellow-400/20 bg-white/10 p-3 shadow-2xl backdrop-blur">
                <CalendlyEmbed />
              </div>
            </div>
          </div>
          ...
          <div className="mx-auto max-w-7xl rounded-[2rem] bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 p-8 text-white shadow-2xl md:p-12">
            <div className="grid gap-10 lg:grid-cols-2">
              ...
              <div className="space-y-4 text-slate-200">
                ...
                <div className="rounded-3xl bg-white/10 p-5"><p className="font-bold text-yellow-400">Official Domain</p><p className="mt-1">jleefantasy.com</p></div>
                <div className="flex flex-col gap-3 sm:flex-row"><LinkButton href="#booking" className="bg-yellow-500 text-slate-950 hover:bg-yellow-400">Book Now</LinkButton><LinkButton href={EMAIL_URL} variant="outline">Email Us</LinkButton></div>
              </div>
            </div>
          </div>
        </section>
